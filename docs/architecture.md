...
| `server/src/repositories/sessions.ts` | MySQL SQL, 행 잠금, 트랜잭션, 통계 집계, 측정 데이터 기록 |
...
`sessions.score`는 여전히 필수 입력입니다. 자세 점수 산식이 없는 지금은 이 값을 만들 수 없으므로, 점수 정책이 정해질 때 NULL 허용 여부를 별도 마이그레이션으로 결정합니다. `bad_posture_seconds`와 휴식 기록은 아직 저장 계약이 없습니다.
